...
function watch(callback::Function, dir::String; targets=ARGS, sources::Union{Vector{Any},Vector{String}}=[])
    (all_files, _start_idx) = get_all_files(dir, String[], targets)
    for src in sources
        for (root, _dirs, files) in walkdir(isfile(src) ? dirname(src) : src)
    ...
    for (_folder, (_fm, loop)) in watched_folders
        schedule(loop)
    end
...
function stop(::typeof(watch))
    for (_folder, (fm, _loop)) in watched_folders
        close(fm)
    end
